...
def format_sections(sections: list[Section]) -> str:
    """ Format a list of sections into a hierarchical string """
    def format_section_recursive(section: Section, level: int = 1, section_num: str = "") -> str:
        # 根据层级设置缩进和编号格式
        indent = "  " * (level - 1)  # 每层缩进2个空格
        
        # 格式化当前章节
        formatted_str = f"""
{'='*60}
{indent}Section {section_num}: {section.name}
{'='*60}
{indent}Description:
{indent}{section.description}
{indent}Requires Research: 
{indent}{section.research}

{indent}Content:
{indent}{section.content if section.content else '[Not yet written]'}
...
"""
        ...
        if section.sections:
            for i, subsection in enumerate(section.sections, 1):
                # 构建子章节编号
                sub_num = f"{section_num}.{i}" if section_num else str(i)
                formatted_str += format_section_recursive(subsection, level + 1, sub_num)
    ...
    for idx, section in enumerate(sections, 1):
        formatted_str += format_section_recursive(section, 1, str(idx))
...
def generate_mindmap(sections: list[Section]) -> str:
    """Generate a hierarchical mindmap of the report sections."""
    
    def generate_mindmap_recursive(section: Section, level: int = 0) -> str:
        # 根据层级设置不同的缩进和符号
        indent = "  " * level
        if level == 0:
            symbol = "- "  # 顶级章节
        ...
        # 生成当前章节的思维导图节点
        mindmap_str = f"{indent}{symbol}{section.name}"
        
        # 如果有子章节，递归生成
        if section.sections:
            mindmap_str += "\n"
            for subsection in section.sections:
                mindmap_str += generate_mindmap_recursive(subsection, level + 1) + "\n"
        
        return mindmap_str.rstrip()  # 移除末尾的换行符
    ...
    for section in sections:
        mindmap_lines.append(generate_mindmap_recursive(section, 0))
